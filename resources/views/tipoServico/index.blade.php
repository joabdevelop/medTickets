            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr class="{{ $tipo_servico->servico_ativo ? '' : 'bloqueio-inativo' }}">
                        ...
                        <td>
                            @if ($tipo_servico->quem_solicita == 0)
                            Equipe interna e Cliente
                            @elseif ($tipo_servico->quem_solicita == 1)
                            Equipe Interna
                            @else
                            Cliente
                            @endif
                        ...
                        <td class="text-center">
                            <div class="form-switch">
                                ...
                                <input class="form-check-input status-toggle" type="checkbox"
                                    ...
                                    data-status="{{ $tipo_servico->servico_ativo ? 1 : 0 }}"
                                    data-bs-toggle="modal"
                                    data-bs-target="#alterarServicoAtivoModal"
                                    role="switch"
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (updateTipoServicoModal) {
                updateTipoServicoModal.addEventListener('show.bs.modal', event => {
                    ...
                    const executante_departamento_id_attr = editButton.getAttribute('data-update_executante_departamento_id');
                    const dados_add = editButton.getAttribute('data-update_dados_add');
                    const servico_ativo = editButton.getAttribute('data-update_servico_ativo'); // Valor booleano
                    ...
                    const servico_ativo_selecionado = updateTipoServicoModal.querySelector('#update_servico_ativo');
                    ...
                    const selectExecutante = updateTipoServicoModal.querySelector('#update_executante_departamento_id');
                    ...
                    if (isNaN(parseInt(executante_departamento_id_attr)) || typeof executante_departamento_id_attr === 'string' && isNaN(executante_departamento_id_attr)) {
            ...
            document.body.addEventListener('change', function(event) {
                ...
                if (event.target && event.target.matches('.status-toggle')) {
                    ...
                    if (form) {
                        ...
                        toggleServicoAtivoModal.dataset.checkboxToRevertId = toggleCheckbox.id || `toggle-temp-${id}`;
